...
], function(mMesaClient, MesaUtils, Deferred, mFileClient, PluginProvider, mServiceregistry, xhr, mGitClient) {
    ...
    var serviceImpl = {
        computeProblems: function(editorContext, options) {
            ...
            xhr("GET", "http://localhost:8080" + options.title + "?parts=meta", {
                ...
                var repoLocation = MesaUtils.repoLocationFromCloneLocation(data.Git.CloneLocation);
                var relativePath = MesaUtils.fileLocationToName(options.title, repoLocation);
